...
Perl_save_scalar(pTHX_ GV *gv)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save SV\n"));
...
Perl_save_generic_svref(pTHX_ SV **sptr)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save GENERIC_SVREF\n"));
...
Perl_save_generic_pvref(pTHX_ char **str)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save GENERIC_PVREF\n"));
...
Perl_save_shared_pvref(pTHX_ char **str)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save SHARED_PVREF\n"));
...
Perl_save_ary(pTHX_ GV *gv)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save AV\n"));
...
Perl_save_hash(pTHX_ GV *gv)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save HV\n"));
...
Perl_save_bool(pTHX_ bool *boolp)
{
    ...
    SS_ADD_END(2);
    DEBUG_lv(Perl_deb(aTHX_ "save BOOL\n"));
}
...
Perl_save_int(pTHX_ int *intp)
{
    ...
    if (UNLIKELY((int)(type >> SAVE_TIGHT_SHIFT) != i)) {
        ...
        type = SAVEt_INT;
        size++;
        DEBUG_lv(Perl_deb(aTHX_ "save INT\n"));
    } else {
        DEBUG_lv(Perl_deb(aTHX_ "save INT_SMALL\n"));
    }
    SS_ADD_PTR(intp);
    SS_ADD_UV(type);
    SS_ADD_END(size);
...
Perl_save_I8(pTHX_ I8 *bytep)
{
    ...
    SS_ADD_END(2);
    DEBUG_lv(Perl_deb(aTHX_ "save I8\n"));
}
...
Perl_save_I16(pTHX_ I16 *intp)
{
    ...
    SS_ADD_END(2);
    DEBUG_lv(Perl_deb(aTHX_ "save I16\n"));
}
...
Perl_save_I32(pTHX_ I32 *intp)
{
    ...
    if (UNLIKELY((I32)(type >> SAVE_TIGHT_SHIFT) != i)) {
        ...
        type = SAVEt_I32;
        size++;
        DEBUG_lv(Perl_deb(aTHX_ "save I32\n"));
    } else {
        DEBUG_lv(Perl_deb(aTHX_ "save I32_SMALL\n"));
    }
    SS_ADD_PTR(intp);
    SS_ADD_UV(type);
    SS_ADD_END(size);
...
Perl_save_strlen(pTHX_ STRLEN *ptr)
{
    ...
    SS_ADD_END(3);
    DEBUG_lv(Perl_deb(aTHX_ "save STRLEN\n"));
}
...
Perl_save_vptr(pTHX_ void *ptr)
{
    ...
    DEBUG_lv(Perl_deb(aTHX_ "save VPTR\n"));
}
...
Perl_leave_scope(pTHX_ I32 base)
{
	...
	case SAVEt_GENERIC_PVREF:		/* generic pv */
	    if (*ARG0_PVP != ARG1_PV) {
                DEBUG_lv(Perl_deb(aTHX_ "restore GENERIC_PVREF '%s' '%s'\n",
                                  *ARG0_PVP, ARG1_PV));
		Safefree(*ARG0_PVP);
		*ARG0_PVP = ARG1_PV;
	    }
	...
	case SAVEt_HV:				/* hash reference */
            DEBUG_lv(Perl_deb(aTHX_ "restore HV %p %p\n", ARG0_HV, ARG1_GV));
	...
	case SAVEt_TMPSFLOOR:			/* restore PL_tmps_floor */
            DEBUG_lv(Perl_deb(aTHX_ "restore TMPSFLOOR %ld => %ld\n",
                              (long)PL_tmps_floor, (long)arg0.any_iv));
	...
	case SAVEt_FREEPADNAME:
            DEBUG_lv(Perl_deb(aTHX_ "restore FREEPADNAME %p\n", ARG0_PTR));
	...
	case SAVEt_CLEARSV:
	    ...
	    break;
	case SAVEt_DELETE:
            DEBUG_lv(Perl_deb(aTHX_ "restore DELETE %p %d %s\n",
                              ARG0_HV, (int)ARG1_I32, ARG2_PV));
	...
	case SAVEt_AELEM:		/* array element */
            DEBUG_lv(Perl_deb(aTHX_ "restore AELEM %p %ld %p\n",
                              ARG0_SV, (long)arg1.any_iv, ARG2_AV));
	...
	case SAVEt_HELEM:		/* hash element */
	    ...
	    HE * const he = hv_fetch_ent(ARG2_HV, ARG1_SV, 1, 0);
            DEBUG_lv(Perl_deb(aTHX_ "restore HELEM %p %p %p\n",
                              ARG0_SV, ARG1_SV, ARG2_HV));
	...
	case SAVEt_PADSV_AND_MORTALIZE:
		...
		svp = AvARRAY((PAD*)ARG1_PTR) + (PADOFFSET)arg0.any_uv;
                DEBUG_lv(Perl_deb(aTHX_ "restore PADSV_AND_MORTALIZE %p[%ld] %p\n",
                                  ARG1_PTR, (long)arg0.any_uv, ARG2_SV));
	...
	case SAVEt_SET_SVFLAGS:
            DEBUG_lv(Perl_deb(aTHX_ "restore SET_SVFLAGS %u %u %p\n",
                              (unsigned int)ARG0_I32, (unsigned int)ARG1_I32, ARG2_SV));
