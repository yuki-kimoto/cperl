...
equivalent to
	...
	die "Not enough arguments for subroutine" unless @_ >= 2;
	my $left = $_[0];
	my $right = $_[1];
	return $left + $right;
    }

In Perl5 an argument can be ignored by omitting the main part of the
...
In cperl this clashes with the protoype syntax and is illegal.
...
Those types must be existing packages. Those additional types
declarations help the compiler to produce better code and to throw
early compile-time errors if possible.
