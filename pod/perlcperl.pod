...
C<feature/CM-712-cperl-types-proto>, and the memory saving branch is
...
CPAN works. 2 modules currently need patches in our distroprefs repo.
...
use no signatures to use C<@_> variables or use sigantures.
...
with %. It is a core module, and is therefore fixed. Testing many other
...
=item B<< Support C<...> for efficient varargs passing >>
...
=item B<< no signatures or lexsubs pragmas required >>
...
code-smell, e.g. SASSIGN and the match functions.
...
switching from chained hash tables to a single ternary trie, radix tree or
DAFSA, without support for C<\0> and maybe even without support for
unicode names.

=head2 Warn on \0shellcode attempts for names (NY)
...
B<Builtin types> are the coretypes Int, Num, Str, UInt, ?Int, ?Num, ?Str
...
precompiled C<ByteCache> F<.pmc> as default. You could pre-compile
