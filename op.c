...
#define const_iv(o) S_const_iv(aTHX_ o)
#define can_const_iv(o) S_can_const_iv(aTHX_ o)
#define op_insert_box(o, o2) S_op_insert_box(aTHX_ o, o2)
#define op_native_variant(o, type1, cast) S_op_native_variant(aTHX_ o, type1, cast)
#define op_upgrade_native(o, c, mod) S_op_upgrade_native(aTHX_ o, c, mod)
#define op_downgrade_native(o, with_box) S_op_downgrade_native(aTHX_ o, with_box)
#define op_can_upgrade_native(o, c) S_op_can_upgrade_native(aTHX_ o, c)
#define op_downgrade_oplist(o, o2) S_op_downgrade_oplist(aTHX_ o, o2)
...
#ifdef USE_ITHREADS
#define	cSVOPo_sv_set(sv) if (cSVOPx(o)->op_sv) cSVOPx(o)->op_sv = (sv); \
    else PAD_SVl((o)->op_targ) = (sv)
#else
#define	cSVOPo_sv_set(sv) cSVOP(o)->op_sv = (sv)
#endif


static OPCODE S_op_native_variant(pTHX_ OP* o, core_types_t type1, int *do_cast);
static bool S_op_upgrade_native(pTHX_ OP* o, OPCODE c, bool mod);
static bool S_op_downgrade_native(pTHX_ OP* o, bool with_box);
...
/* Returns TRUE if the CONST op contains an IV value */
 ...
 * Returns IV_MAX on error. Better check with S_can_const_iv() before.
 */
...
S_check_hash_fields_and_hekify(pTHX_ UNOP *rop, SVOP *key_op)
{
    ...
    if (rop) {
        ...
        if (IS_PADSV_OP(rop->op_first))
            /* @$hash{qw(keys here)} */
            rop = (UNOP*)rop->op_first;
        else {
            /* @{$hash}{qw(keys here)} */
            if (rop->op_first->op_type == OP_SCOPE
                && IS_PADSV_OP(cLISTOPx(rop->op_first)->op_last)) {
                  ...
                  OP *block, bool o_is_gv)
{
	...
	block->op_next = 0;
        if (ps && !*ps && !attrs && !CvLVALUE(PL_compcv))
            const_sv = S_op_const_sv(aTHX_ start, PL_compcv,
...
/* Check for shaped arrays and types.
...
/* We should not downgrade a native pad.
   So when we can insert a BOX op, do not downgrade a pad.
...
S_op_downgrade_oplist(pTHX_ OP* o, OP* o2)
{
    ...
    for (; o && o != o2; o=o->op_next) {
        if (IS_NATIVE_OP(o)) op_downgrade_native(o, TRUE);
    }
}
            ...
            if (v) {
                ...
                if (oc == OA_UNOP || oc == OA_BASEOP_OR_UNOP) {
                    ...
                    if (typeret==type1 && match_type2(n2 & 0xffffff00, type1, type2)) {
                        ...
                        if (typ == OP_DIVIDE && v == OP_I_DIVIDE)
                            continue;
                        /* perl5 features a "wild" interpretation of modulo with negative args,
                           which clashes with libc and i_modulo */
                        if (typ == OP_MODULO && v == OP_I_MODULO
...
#endif
            if (o->op_sibling && o->op_sibling == o->op_next) {
                o->op_next = newOP(v, 0);
                o->op_sibling = o->op_next;
                o = o->op_next;
                o->op_next = o->op_sibling = o2;
                o->op_moresib = 1;
            } else {
                o->op_next = newOP(v, 0);
                if (o->op_sibling) {
                    o->op_next->op_sibling = o->op_sibling;
                    o->op_sibling = o->op_next;
                    o->op_moresib = 1;
                }
                o->op_next->op_next = o2;
            }
        }
    }
}
...
Perl_rpeep(pTHX_ OP *o)
{
	...
	if (!o) {
      ...
      redo:
        ...
        assert(!oldoldop || !oldop || oldoldop->op_next == oldop);
	...
	PL_op = o;
        ...
        if (IS_NATIVE_OP(o) || IS_NATIVE_PADSV(o)) {
            ...
            for (o2 = o->op_next, do_cast = 1;
                 o2 && ((is_native = IS_NATIVE_OP(o2)) || (v = op_native_variant(o2, t, &do_cast)));
                 o2 = o2->op_next, do_cast = 1)
            {
                if (!is_native) { /* convert it */
	...
	case OP_NEXTSTATE:
		...
		if (   OP_TYPE_IS(next, OP_PUSHMARK)
		    ...
		    && ( OP_TYPE_IS(sibling->op_next->op_next, OP_LEAVESUB)
		       ||OP_TYPE_IS(sibling->op_next->op_next, OP_LEAVESUBLV))
